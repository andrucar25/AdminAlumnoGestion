...
export default function JuradoMenuA(props) {
  ...
  useEffect(() => {
    if (IsMounted.current == false) {
      Axios.get(`http://${env.ip}:${env.port}/orden/${props.route.params.id}`).then(response => {
        ...
        const result = response.data.filter(e => e.categoria == "Categoria C");
        setcategoria(result)


        //console.log(categoria[0].ordenProyectos[0].catA)
        setcategoriaC(result[0].ordenProyectos[0].catC)
    ...
    <>
      <View
        ...
        <ScrollView style={{
          ...
          {categoriaC.map(card => <Card nombrePr={card.nombreProyecto} curso={card.curso} id={card._id} />)}
