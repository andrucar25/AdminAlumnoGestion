  ...
  useEffect(() => {
    if (IsMounted.current == false) {
      Axios.get(`http://${env.ip}:${env.port}/orden/${props.route.params.id}`).then(response => {
        ...
        const result = response.data.filter(e => e.categoria == "Categoria A");
        setcategoria(result)


        //console.log(categoria[0].ordenProyectos[0].catA)
        setcategoriaA(result[0].ordenProyectos[0].catA)
    ...
    <>
      <View
        ...
        <ScrollView style={{
          ...
          {categoriaA.map(card => <Card nombrePr={card.nombreProyecto} curso={card.curso} id={card._id} />)}
